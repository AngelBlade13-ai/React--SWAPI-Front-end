...
const StarshipComponent = ({ searchQuery }) => {
  ...
  useEffect(() => {
    GetStarships().then((response) => {
      const starshipList = response?.data?.results ?? [];
      setStarships(starshipList);
      setFilteredStarships(starshipList);
    });
  }, []);
            ...
            <Card key={index} className="shadow-sm">
              <Card.Body className="d-flex justify-content-between align-items-center">
                <div>
                  ...
                  <Card.Text className="text-muted">Starship ID: {starship.uid}</Card.Text>
                </div>
                <Link to={`/ViewStarship/${starship.uid}`} className="btn btn-outline-primary">
                  View Details
                </Link>
              </Card.Body>
            </Card>
