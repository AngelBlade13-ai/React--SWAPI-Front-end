...
const PlanetComponent = ({ searchQuery }) => {
  ...
  useEffect(() => {
    GetPlanets().then((response) => {
      const planetList = response?.data?.results ?? [];
            ...
            <Card key={index} className="shadow-sm">
              <Card.Body className="d-flex justify-content-between align-items-center">
                <div>
                  ...
                  <Card.Text className="text-muted">Planet ID: {planet.uid}</Card.Text>
                </div>
                <Link to={`/ViewPlanet/${planet.uid}`} className="btn btn-outline-primary">
                  View Details
                </Link>
              </Card.Body>
            </Card>
